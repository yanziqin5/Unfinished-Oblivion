...
from widgets.sponsor_dialog import SponsorDialog
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.toolbar.sponsor_requested.connect(self._show_sponsor)
    ...
    def _show_sponsor(self):
        """爱发电赞助弹窗。"""
        dlg = SponsorDialog(self, getattr(self, "_dark_mode", False))
        dlg.move(
            self.x() + (self.width() - dlg.width()) // 2,
            self.y() + (self.height() - dlg.height()) // 2,
        )
